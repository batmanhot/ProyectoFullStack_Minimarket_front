...
export const saleService = {
  ...
  async create(payload) {
    ...
    // allocateStock decide automáticamente la estrategia según product.stockControl.
    // El cajero nunca elige lotes — el motor lo hace de forma transparente.
    ...
    for (const item of payload.items) {
      ...
      if (result.error) {
        // Si el motor falla (sin stock), continuamos sin descontar
        // En producción esto sería un rollback — aquí logueamos y seguimos
      ...
      // Aplicar actualización de stock y lotes al producto
      freshState.updateProduct(item.productId, result.stockUpdate)

      // Registrar movimiento con info completa del motor
      ...
      // Enriquecer el ítem con info de lotes asignados (para el ticket)
      enrichedItems.push({
        ...item,

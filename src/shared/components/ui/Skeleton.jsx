export function TableSkeleton({ rows = 8, cols = 5 }) {
  return (
    <div className="animate-pulse">
      {[...Array(rows)].map((_, i) => (
        <div key={i} className="flex gap-4 py-3 border-b border-gray-50" style={{ opacity: 1 - i * 0.08 }}>
          {[...Array(cols)].map((_, j) => (
            <div key={j} className="h-4 bg-gray-100 rounded flex-1" style={{ maxWidth: j === 0 ? '200px' : undefined }} />
          ))}
        </div>
      ))}
    </div>

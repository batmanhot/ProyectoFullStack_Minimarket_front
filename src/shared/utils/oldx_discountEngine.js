...
export function evaluateDiscounts(cartItems, products, campaigns) {
  ...
  // Contador de aplicaciones por campaña
  const campaignUsage = new Map()
  ...
  for (const campaign of activeCampaigns) {
    // Verificar límite máximo de usos por compra
    const maxUses = campaign.maxUsesPerSale || 2
    const currentUses = campaignUsage.get(campaign.id) || 0
    if (currentUses >= maxUses) continue // Saltar si ya alcanzó límite

    // ── CAMPAÑAS QUE APLIQUE A PRODUCTOS (TODOS LOS TIPOS) ──────────────────
    let itemSaving = 0
    let campaignItemsApplied = 0
    ...
    itemDiscounts.forEach(item => {
      ...
      if (disc > 0 && campaignItemsApplied < maxUses) {
        ...
        item.discountDetails.push({
          ...
          description: `${campaign.description || ''} (uso ${currentUses + 1}/${maxUses})`
        })
        itemSaving += disc
        campaignItemsApplied++
      }
    })
    
    // Registrar uso de esta campaña
    campaignUsage.set(campaign.id, currentUses + campaignItemsApplied)
...
export const DAYS_OF_WEEK = [
  ...
  export const CAMPAIGN_TEMPLATES = [
    ...
    { name: '2x1 Producto destacado',   icon: '🎁', type: 'promotion', buyQty: 2, payQty: 1, scope: 'products', maxUsesPerSale: 2 },
    { name: '3x2 Temporada',            icon: '🎁', type: 'promotion', buyQty: 3, payQty: 2, scope: 'all', maxUsesPerSale: 3 },

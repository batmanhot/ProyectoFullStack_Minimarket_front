  ...
  subscribeWithSelector(
    persist(
      ...
      }),
      {
        ...
        // ── MERGE PERSONALIZADO ──────────────────────────────────────────────
        // Zustand por defecto hace un shallow merge entre el estado persistido
        // y el estado inicial. Eso causa que los campos loyalty (loyaltyPoints,
        // loyaltyAccumulated, loyaltyTransactions) se pierdan si el cliente en
        // el estado inicial (seed) no los tiene definidos.
        //
        // Este merge personalizado preserva los campos loyalty del localStorage
        // al fusionar los clientes, garantizando que los puntos acumulados
        // sobrevivan recargas de página.
        merge: (persistedState, currentState) => {
          const merged = { ...currentState, ...persistedState }

          // Preservar campos loyalty del localStorage para cada cliente
          if (persistedState?.clients && currentState?.clients) {
            const persistedMap = {}
            persistedState.clients.forEach(c => { persistedMap[c.id] = c })

            merged.clients = currentState.clients.map(seedClient => {
              const persisted = persistedMap[seedClient.id]
              if (!persisted) return seedClient
              // Usar datos del localStorage, pero garantizar que los campos
              // loyalty estén inicializados si el cliente persistido no los tiene
              return {
                ...seedClient,
                ...persisted,
                loyaltyPoints:       persisted.loyaltyPoints       ?? seedClient.loyaltyPoints       ?? 0,
                loyaltyAccumulated:  persisted.loyaltyAccumulated  ?? seedClient.loyaltyAccumulated  ?? 0,
                loyaltyLevel:        persisted.loyaltyLevel         ?? seedClient.loyaltyLevel         ?? 'Bronce',
                loyaltyTransactions: persisted.loyaltyTransactions  ?? seedClient.loyaltyTransactions  ?? [],
              }
            })

            // Agregar clientes nuevos que solo están en localStorage (no en el seed)

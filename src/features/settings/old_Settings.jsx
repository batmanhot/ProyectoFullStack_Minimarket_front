...
export default function Settings() {
  const { businessConfig, systemConfig, updateBusinessConfig, updateSystemConfig, loadDemoData } = useStore()
        ...
        <div className="flex items-start justify-between gap-6 flex-wrap">
          <div className="flex-1 min-w-60">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-xs px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-400 font-medium">
                Para presentaciones
              </span>
            </div>
            <p className="text-xs text-blue-700 dark:text-blue-400 leading-relaxed">
              Inyecta <strong>30 días de ventas históricas</strong> con variedad de métodos de pago
              (efectivo, Yape, tarjeta, crédito), descuentos de campaña aplicados y horarios realistas
              de tráfico. Así el Dashboard, Reportes y Caja se ven ricos desde el primer momento
              del demo — sin necesidad de generar datos manualmente.
            </p>
            <p className="text-xs text-blue-500 dark:text-blue-500 mt-2">
              ⚠️ Solo reemplaza el historial de ventas. Los productos, clientes y configuración
              no se modifican.
            </p>
          </div>
          <button
            onClick={async () => {
              ...
              try {
                loadDemoData()
                toast.success('✅ Datos demo cargados — Dashboard y Reportes listos', { duration: 4000 })
              } catch (e) {

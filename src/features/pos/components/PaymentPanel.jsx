  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 border-b border-gray-100 dark:border-slate-700">
        <p className="text-xs font-medium text-gray-400 dark:text-slate-500 uppercase tracking-wide mb-2">Cliente (opcional)</p>
        {selectedClient ? (
          <div className="flex items-center gap-2 bg-blue-50 rounded-lg px-3 py-2">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-xs font-medium text-gray-800 dark:text-slate-100 truncate">{selectedClient.name}</div>
              {selectedClient.creditLimit > 0 && (
                <div className="text-xs text-blue-600">Crédito disp: {formatCurrency(creditAvailable)}</div>
              )}
            </div>
            <button onClick={() => { setClientId(null); setClientSearch('') }} className="text-gray-400 dark:text-slate-500 hover:text-red-400 text-xs">✕</button>
          </div>
        ) : (
          <div className="relative">
            ...
            {showClientSearch && filteredClients.length > 0 && (
              <div className="absolute top-full mt-1 left-0 right-0 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-600 rounded-lg shadow-lg z-20 max-h-40 overflow-y-auto">
                {filteredClients.map(c => (
                  <button key={c.id} onMouseDown={() => { setClientId(c.id); setClientSearch(c.name); setShowClientSearch(false) }}
                    className="w-full text-left px-3 py-2 text-sm hover:bg-blue-50 border-b border-gray-50 last:border-0">
                    <div className="font-medium text-gray-800 dark:text-slate-100">{c.name}</div>
                    <div className="text-xs text-gray-400 dark:text-slate-500">{c.documentType} {c.documentNumber}{c.creditLimit > 0 ? ` · Crédito: ${formatCurrency(creditAvailable)}` : ''}</div>
                  </button>
                ))}
              </div>
            )}
      ...
      <div className="p-3 border-b border-gray-100 dark:border-slate-700">
        <div className="flex justify-between text-sm font-semibold text-gray-800 dark:text-slate-100 mb-2">
        ...
        {payments.map((p, i) => {
          ...
          return (
            <div key={i} className="flex items-center gap-2 text-xs py-1">
              <span>{m?.icon}</span>
              <span className="flex-1 text-gray-600 dark:text-slate-300">{m?.label}{p.reference ? ` — ${p.reference}` : ''}</span>
              <span className="font-medium text-gray-800 dark:text-slate-100">{formatCurrency(p.amount)}</span>
      ...
      <div className="p-3 border-b border-gray-100 dark:border-slate-700 flex-1 overflow-y-auto">
        ...
        <div className="grid grid-cols-4 gap-1.5 mb-3">
          {PAYMENT_METHODS.map(pm => (
            <button key={pm.value} onClick={() => setCurrentMethod(pm.value)}
              className={`flex flex-col items-center gap-0.5 py-2 rounded-lg border text-xs font-medium transition-all ${currentMethod === pm.value ? 'border-blue-500 bg-blue-50 text-blue-700' : 'border-gray-200 dark:border-slate-600 text-gray-500 dark:text-slate-400 hover:border-gray-300'}`}>
        ...
        {currentMethod === 'efectivo' && (
          <div className="grid grid-cols-5 gap-1.5">
            ...
            {BILLETES_PEN.filter(b => b >= total).slice(0, 3).map(b => (
              <button key={b} onClick={() => quickAmount(b)} className="py-1 text-xs border border-gray-200 dark:border-slate-600 rounded text-gray-600 dark:text-slate-300 hover:bg-gray-50 dark:bg-slate-800/50 dark:hover:bg-slate-700">
                S/{b}
              </button>
            ))}
          </div>
        ...
        {(currentMethod === 'yape' || currentMethod === 'plin') && (
          <div className="mt-2 bg-gray-50 dark:bg-slate-800/50 rounded-lg p-3 text-center">
            ...
            <p className="text-xs text-gray-500 dark:text-slate-400">Mostrar QR al cliente</p>
            <p className="text-xs text-gray-400 dark:text-slate-500">Luego ingresar el código de operación arriba</p>
          </div>
        )}
      </div>

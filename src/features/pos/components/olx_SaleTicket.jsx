...
function buildTicketHTML(sale, businessConfig, storeProducts = []) {
  ...
  const itemsHtml = (sale.items || []).filter(i => !i._fromBundle && !i.fromBundle).map(item => {
    ...
    <div style="margin-bottom:4px">
      ...
      ${(() => {
        const serial = item.selectedSerial || item.batchAllocations?.[0]?.batchNumber
        if (serial && item.stockControlUsed === 'serie') return '<div style="font-size:8px;color:#7c3aed;margin-left:2px;font-family:monospace">Serie: ' + serial + '</div>'
        return ''
      })()}
      ${(() => {
              ...
              {(sale.items || []).filter(item => !item._fromBundle && !item.fromBundle).map((item, idx) => {
                ...
                return (
                  <div key={idx} style={{ marginBottom: '8px' }}>
                    ...
                    {/* Serial vendido — productos tipo SERIE */}
                    {(() => {
                      const serial = item.selectedSerial || item.batchAllocations?.[0]?.batchNumber
                      if (!serial || item.stockControlUsed !== 'serie') return null
                      return (
                        <div style={{ fontSize: '9px', color: '#7c3aed', marginTop: '2px', fontFamily: 'monospace', letterSpacing: '0.02em' }}>
                          Serie: {serial}
                        </div>
                      )
                    })()}

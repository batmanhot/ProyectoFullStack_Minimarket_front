...
function ProductsView({ products, categories, brands, suppliers, businessConfig, addAuditLog }) {
  ...
  const handleNewProduct = () => {
    if (atProductLimit) {
      toast.error(`Tu plan permite máximo ${maxProducts} producto${maxProducts !== 1 ? 's' : ''}. Actualiza tu plan para agregar más.`, { duration: 6000 })
      return
    }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex gap-2 flex-wrap items-center">
        ...
        {maxProducts !== null && (
          <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${atProductLimit ? 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400' : 'bg-gray-100 dark:bg-slate-700 text-gray-500 dark:text-slate-400'}`}>
            {products.length}/{maxProducts} productos
          </span>
        )}
        <button onClick={handleNewProduct}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${atProductLimit ? 'bg-gray-200 dark:bg-slate-700 text-gray-400 dark:text-slate-500 cursor-not-allowed' : 'bg-blue-600 text-white hover:bg-blue-700'}`}>

...
function SupplierForm({ supplier, onClose }) {
  ...
  const cls = "w-full px-3 py-2 border border-gray-200 dark:border-slate-600 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div className="col-span-2"><label className="block text-xs font-medium text-gray-600 dark:text-slate-300 mb-1">Nombre / Razón social *</label><input {...register('name')} className={cls}/>{errors.name && <p className="text-xs text-red-500 mt-1">{errors.name.message}</p>}</div>
        <div><label className="block text-xs font-medium text-gray-600 dark:text-slate-300 mb-1">RUC / N° tributario</label><input {...register('taxId')} className={cls}/></div>
        <div><label className="block text-xs font-medium text-gray-600 dark:text-slate-300 mb-1">Contacto</label><input {...register('contact')} className={cls}/></div>
        <div><label className="block text-xs font-medium text-gray-600 dark:text-slate-300 mb-1">Teléfono</label><input {...register('phone')} className={cls}/></div>
        <div><label className="block text-xs font-medium text-gray-600 dark:text-slate-300 mb-1">Email</label><input {...register('email')} type="email" className={cls}/>{errors.email && <p className="text-xs text-red-500 mt-1">{errors.email.message}</p>}</div>
        <div className="col-span-2"><label className="block text-xs font-medium text-gray-600 dark:text-slate-300 mb-1">Dirección</label><input {...register('address')} className={cls}/></div>
        <div className="col-span-2"><label className="block text-xs font-medium text-gray-600 dark:text-slate-300 mb-1">Notas</label><textarea {...register('notes')} rows={2} className={cls + ' resize-none'}/></div>
      </div>
...
export default function Suppliers() {
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar por nombre o RUC..." className="w-full max-w-md px-3 py-2 border border-gray-200 dark:border-slate-600 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
      ...
      {filtered.length === 0 ? (
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700 overflow-hidden">
          <table className="w-full">
            <thead><tr className="bg-gray-50 dark:bg-slate-800/50 border-b border-gray-100 dark:border-slate-700">{['Proveedor','Contacto','Productos','Total comprado','Acciones'].map(h => <th key={h} className={`text-xs font-medium text-gray-500 dark:text-slate-400 px-4 py-3 ${['Productos','Total comprado'].includes(h)?'text-right':h==='Acciones'?'text-center':'text-left'}`}>{h}</th>)}</tr></thead>
            <tbody className="divide-y divide-gray-50 dark:divide-slate-700/50">
              {filtered.map(s => {
                const st = getStats(s.id)
                return (
                  <tr key={s.id} className="hover:bg-gray-50 dark:bg-slate-800/50 dark:hover:bg-slate-700">
                    <td className="px-4 py-3"><div className="text-sm font-medium text-gray-800 dark:text-slate-100">{s.name}</div>{s.taxId && <div className="text-xs text-gray-400 dark:text-slate-500">RUC: {s.taxId}</div>}</td>
                    <td className="px-4 py-3"><div className="text-xs text-gray-600 dark:text-slate-300">{s.contact||'—'}</div><div className="text-xs text-gray-400 dark:text-slate-500">{[s.phone,s.email].filter(Boolean).join(' · ')}</div></td>
                    <td className="px-4 py-3 text-right text-sm font-medium text-gray-700">{st.productCount}</td>
                    <td className="px-4 py-3 text-right"><span className="text-sm font-medium text-gray-700">{formatCurrency(st.totalComprado)}</span><div className="text-xs text-gray-400 dark:text-slate-500">{st.purchaseCount} compras</div></td>
                    <td className="px-4 py-3 text-center">
                      <button onClick={() => setModal({ type: 'form', data: s })} className="p-1.5 text-gray-400 dark:text-slate-500 hover:text-amber-600 hover:bg-amber-50 rounded-lg" title="Editar">

  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-medium text-gray-800">Auditoría del sistema</h1>
          <p className="text-sm text-gray-400">{filtered.length} de {kpis.total} registros · trazabilidad completa de operaciones</p>
        </div>
        <div className="flex gap-2">
          <button onClick={handleExport}    className="px-3 py-2 text-sm border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-50">📊 Excel</button>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gray-50 rounded-xl p-4"><p className="text-xs text-gray-500 mb-1">Total de eventos</p><p className="text-2xl font-medium text-gray-800">{kpis.total}</p></div>
      ...
      <div className="flex gap-3 flex-wrap">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Buscar en detalle, usuario, módulo..." className="flex-1 min-w-48 px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"/>
        <select value={moduleF} onChange={e => setModuleF(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
          {MODULES.map(m => <option key={m} value={m}>{m}</option>)}
        </select>
        <select value={actionF} onChange={e => setActionF(e.target.value)} className="px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <button onClick={() => { setSearch(''); setModuleF('Todos'); setActionF('Todas'); setDateFrom(''); setDateTo('') }} className="px-3 py-2 text-xs text-gray-500 border border-gray-200 rounded-lg hover:bg-gray-50">Limpiar</button>
      </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-50">
              {filtered.slice(0, 200).map(entry => {
                const actionCfg = AUDIT_ACTIONS[entry.action] || { label: entry.action, color: 'bg-gray-100 text-gray-600' }
                return (
                  <tr key={entry.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-4 py-2.5 text-xs text-gray-500 whitespace-nowrap">{formatDateTime(entry.timestamp)}</td>
                    <td className="px-4 py-2.5">
                      <div className="text-sm font-medium text-gray-800">{entry.userName || 'Sistema'}</div>
                    </td>
                    <td className="px-4 py-2.5">
                      <span className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded-full">{entry.userRole}</span>
                    </td>
                    <td className="px-4 py-2.5">
                    ...
                    <td className="px-4 py-2.5 text-sm text-gray-600 max-w-xs">{entry.detail}</td>
                    <td className="px-4 py-2.5 text-xs font-mono text-gray-400 max-w-[100px] truncate">{entry.entityId ? entry.entityId.slice(-8) : '—'}</td>
                  </tr>
                )
              })}
            </tbody>
